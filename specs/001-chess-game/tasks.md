...
- [ ] T020 [P] [US1] Implement pawn movement validation in src/lib/chess-rules.ts (forward 1/2 squares, diagonal capture)
- [ ] T021 [P] [US1] Implement rook movement validation in src/lib/chess-rules.ts (horizontal/vertical lines)
- [ ] T022 [P] [US1] Implement knight movement validation in src/lib/chess-rules.ts (L-shaped moves)
- [ ] T023 [P] [US1] Implement bishop movement validation in src/lib/chess-rules.ts (diagonal lines)
- [ ] T024 [P] [US1] Implement queen movement validation in src/lib/chess-rules.ts (rook + bishop combined)
- [ ] T025 [P] [US1] Implement king movement validation in src/lib/chess-rules.ts (one square any direction)
- [ ] T026 [US1] Implement isPathClear function in src/lib/chess-rules.ts (check obstacles between positions)
- [ ] T027 [US1] Implement getLegalMoves function in src/lib/move-generator.ts (generate pseudo-legal moves for piece)
- [ ] T028 [US1] Implement check detection function in src/lib/chess-rules.ts (isKingInCheck)
- [ ] T029 [US1] Implement move validation with check prevention in src/lib/move-generator.ts (filter moves that expose king)
- [ ] T030 [US1] Implement checkmate detection function in src/lib/chess-rules.ts (no legal moves available while in check)
- [ ] T031 [US1] Implement stalemate detection function in src/lib/chess-rules.ts (no legal moves available, not in check)
- [ ] T032 [P] [US1] Create ChessPiece component in src/components/ChessPiece.tsx (render Unicode symbols)
- [ ] T033 [P] [US1] Create ChessSquare component in src/components/ChessSquare.tsx (render single square with click handler)
- [ ] T034 [US1] Create ChessBoard component in src/components/ChessBoard.tsx (8x8 grid of squares)
- [ ] T035 [US1] Create GameStatus component in src/components/GameStatus.tsx (turn indicator and game end message)
- [ ] T036 [US1] Implement game state management with React Context in src/lib/game-state.ts (useGameState hook)
- [ ] T037 [US1] Implement piece selection logic in src/lib/game-state.ts (handle square click, update selectedPiece)
- [ ] T038 [US1] Implement move execution logic in src/lib/game-state.ts (validate and apply move, toggle turn)
- [ ] T039 [US1] Integrate game state updates with checkmate/stalemate detection in src/lib/game-state.ts
- [ ] T040 [US1] Create main chess game page in src/app/page.tsx (integrate ChessBoard and GameStatus)
- [ ] T041 [US1] Add global styles for board layout in src/app/globals.css (grid, colors, spacing)
...
- [ ] T064 [P] Add basic keyboard accessibility in src/components/ChessSquare.tsx (exploratory: Tab focus, Enter to interact - refine based on testing)
...
- [ ] T070 Validate all acceptance scenarios from spec.md (manual testing checklist including performance: initial load <2s, move feedback <100ms perceived responsiveness)
